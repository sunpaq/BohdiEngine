...
static MCPoint mc_point(double x, double y)
{
    MCPoint point = {x, y};
    return point;
}

static MCPoint* mc_point_copy(MCPoint* A, MCPoint B)
{
    A->x = B.x;
    A->y = B.y;
    return A;
...
static MCSize mc_size(double width, double height)
{
    MCSize point = {width, height};
    return point;
}

static MCRect mc_rect(double x, double y, double width, double height)
{
    MCRect frame = {{x, y}, {width, height}};
    return frame;
}

static int mc_rect_contains(MCRect* rect, MCPoint point)
{
...
static MCColor mc_color(unsigned R, unsigned G, unsigned B)
{
    MCColor color = {R, G, B};
    return color;
}

static MCColor mc_color_mix(MCColor A, MCColor B)
{
